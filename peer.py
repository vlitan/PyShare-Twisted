...
def peer_main():
    ...
    def get_package(network, index, switch_address):
        ...
        #    if port != options.server_port: #TODO replace port with host when running in cloud
            if host != socket.gethostname(): # if not connectig with self
                d = defer.Deferred()
                # no switch implementation
                # factory = client.MyClientFactory(d, packages, index, gotDone)
                # reactor.connectTCP(host, int(port), factory)

                # switch implementation
                factory = client.MyClientFactory(d, packages, index, address, gotDone)
                host, port = switch_address.split(':')
                reactor.connectTCP(host, int(port), factory)
                defers.append(d)
        return defers
    ...
    switch_address = "10.142.0.5:8000"
    NETWORK = [  "10.142.0.2:5002"
                ,"10.142.0.3:5003"
                ,"10.142.0.4:5004"]
    # switch_address = "localhost:8000"
    ...
    for i in range(len(packages)):
        if packages[i] is None:
            defers.extend(get_package(NETWORK, i, switch_address))
